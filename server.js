...
import { Low, JSONFile } from 'lowdb';   // ✅ universal import (no /node)
...
app.post('/api/register', async (req, res) => {
  ...
  await db.read();
  if (db.data.users.find(u => u.username === username || u.userId === userId))
...
app.post('/api/login', async (req, res) => {
  ...
  const user = db.data.users.find(
    u => (u.username === username && u.userId === userId) || (u.userId === userId && !username)
  );
...
app.post('/api/login-face', async (req, res) => {
  ...
  for (const user of db.data.users) {
    ...
    for (const d of user.faceDescriptors) {
      ...
      for (let i = 0; i < d.length; i++) {
        const diff = (d[i] || 0) - (descriptor[i] || 0);
        sum += diff * diff;
      }
  ...
  if (best.user && best.distance <= MATCH_THRESHOLD) {
    const token = createToken({ sub: best.user.id });
    return res.json({
      token,
      user: { id: best.user.id, username: best.user.username, userId: best.user.userId },
...
app.post('/api/sync', async (req, res) => {
  ...
  // Keep last 5 backups per user
  const userBackups = db.data.backups
    .filter(b => b.ownerId === payload.sub)
    .sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  db.data.backups = db.data.backups
    .filter(b => b.ownerId !== payload.sub)
    .concat(userBackups.slice(0, 5));
...
// === Serve HTML Pages ===
const pages = ['cards', 'transactions', 'grace', 'settings', 'login'];
pages.forEach(page => {
...
app.listen(PORT, () => console.log(`GraceWise v5.7.1 running on port ${PORT}`));
